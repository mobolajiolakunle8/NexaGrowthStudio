...
// Declarations in ../declarations.d.ts are ambient — no need to import.
...
export default function App() {
  ...
  // ── Live sync ──
  useEffect(() => {
    const unsubscribe = startLiveSync((fbooks: Book[], _fleads: Lead[]) => {
      setBooks(fbooks);
      saveBooks(fbooks);
    });
        ...
        <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center px-6" onClick={() => setMegaPasscodeOpen(false)}>
          <div className="bg-slate-900 border border-slate-800 text-slate-100 rounded-2xl p-8 max-w-sm w-full shadow-2xl relative" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleMegaLogin} className="flex flex-col gap-3">
              <input required type="password" placeholder="Enter mega admin passcode" value={megaCode} onChange={e => setMegaCode(e.target.value)} className="bg-slate-950 border border-slate-800 px-4 py-3 rounded-lg text-sm text-white focus:outline-none focus:ring-1 focus:ring-amber-500 text-center tracking-widest font-mono" autoFocus />
              {megaErr && <p className="text-xs text-red-400 text-center">{megaErr}</p>}
              <button type="submit" className="bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold text-sm py-3 rounded-full mt-1 transition">Unlock Publishing HQ</button>
            </form>
