...
export default function App() {
  ...
  useEffect(() => {
    void (async () => {
      try {
        ...
        const [cloud, cloudArticles] = await Promise.all([
          bootstrapCloud(localBooks.length ? localBooks : [SEED_BOOK]),
          fetchArticlesOnce().catch(() => [] as Article[]),
        ]);
        setBooks(cloud.books);
        saveBooks(cloud.books);
        if (cloud.settings) {
          setSiteSettings(cloud.settings);
          if (!localStorage.getItem('nexa_public_theme') && cloud.settings.defaultTheme) {
            setPublicTheme(cloud.settings.defaultTheme);
          }
        }
  ...
  // ── Live articles sync (every browser updates instantly) ──
  useEffect(() => {
    const stop = startArticlesSync((cloudArticles) => {
      setArticles(cloudArticles);
      saveArticles(cloudArticles);
    });
    return stop;
  ...
  const handlePublicTheme = useCallback((next: 'light' | 'dark') => {
    setPublicTheme(next);
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem('nexa_public_theme');
      if (saved !== 'light' && saved !== 'dark' && siteSettings.defaultTheme) {
        setPublicTheme(siteSettings.defaultTheme);
      }
    } catch { /* ignore */ }
  ...
  const renderMain = () => {
    ...
    // Articles newsroom (separate page — never embedded on the home page)
    if (route.name === 'articles') {
      return (
        <ArticlesHome
          ...
          settings={siteSettings}
          theme={publicTheme}
          onToggleTheme={() => handlePublicTheme(publicTheme === 'dark' ? 'light' : 'dark')}
        />
      );
    }
    ...
    if (route.name === 'article') {
      ...
      const related = articles.filter(a => a.published !== false && a.slug !== route.slug).slice(0, 3);
      if (!article) return <NotFound onBack={() => { window.location.hash = '/articles'; }} />;
      return (
        <ArticleDetail
          article={article}
          related={related}
