...
/* ────────────────────────────────────────────────────────────────
   ...
   ──────────────────────────────────────────────────────────────── */
/* ────────────────────────────────────────────────────────────────
   Template: New article alert (with cover image) — to one subscriber
   ──────────────────────────────────────────────────────────────── */
...
export function newArticleTemplate(article: Article, subscriberEmail: string, settings: SiteSettings) {
  const excerpt = article.excerpt || article.body.split(/\n\s*\n/)[0]?.slice(0, 220) || '';
  const hero = article.coverImage
    ? `<tr><td style="padding:0 0 22px;"><img src="${article.coverImage}" alt="${escapeHtml(article.title)}" width="100%" style="display:block;width:100%;max-width:548px;height:auto;border-radius:16px;" /></td></tr>`
    : '';
  const meta = [article.category, article.readMinutes ? `${article.readMinutes} min read` : '', article.author]
    .filter(Boolean)
    .map(escapeHtml)
    .join(' &nbsp;·&nbsp; ');
  ...
  const body = `
    ...
    <p style="margin:0 0 12px;font-family:'JetBrains Mono',monospace;font-size:11px;letter-spacing:.14em;text-transform:uppercase;color:${BRAND.ochre};">${meta}</p>
    <p style="margin:0 0 18px;">${escapeHtml(excerpt)}${excerpt.length >= 220 ? '…' : ''}</p>
  `;

  return {
    html: wrapEmail({
      preheader: `New article: ${article.title}`,
      kicker: 'New article alert',
      heading: article.title,
      bodyHtml: hero.replace('<tr><td style="padding:0 0 22px;">', '<div style="padding:0 0 22px;">').replace('</td></tr>', '</div>') + body,
